...
class Lightbox extends Component {
  ...
  handleKeyUp = e => {
    ...
    if (this.state.showLightbox) {
      ...
      if (keyCode === 39) {
        // Right Arrow Key
        const { images, folderName } = this.props;
        if (this.state.selectedImage < images.filter(function (str) { return str.node.sizes.src.indexOf(folderName) >= 0 }).length - 1) {
          this.setState({ selectedImage: this.state.selectedImage + 1 })
        }
      }
      if (keyCode === 27) {
  ...
  render() {
    ...
    const finalImages = images.filter(function (str) { return str.node.sizes.src.indexOf(folderName) >= 0 });
    return (
      <Fragment>
        <Gallery>
          ...
          {finalImages[selectedImage] && <Img sizes={finalImages[selectedImage].node.sizes} />}
            <Controls>
              ...
              <LeftRight>
                ...
                <Button onClick={this.goForward} disabled={selectedImage === finalImages.length - 1}>
                  Next
                </Button>
              </LeftRight>
            </Controls>
